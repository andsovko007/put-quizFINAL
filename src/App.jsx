...
const IS_TG = !!TG;
const TG_BOT_LINK = "https://t.me/Sovkoandreiwaybot";
const RESULT_SENT_KEY = "put_quiz_result_sent";
...
function isResultAlreadySent() {
  try {
    return window.sessionStorage.getItem(RESULT_SENT_KEY) === "1";
  } catch {
    return false;
  }
}

function markResultSent() {
  try {
    window.sessionStorage.setItem(RESULT_SENT_KEY, "1");
  } catch {}
}

function clearResultSent() {
  try {
    window.sessionStorage.removeItem(RESULT_SENT_KEY);
  } catch {}
}

function sendResultToBotOnce(payload) {
  if (!IS_TG || !TG?.sendData || !payload) return false;
  if (isResultAlreadySent()) return true;
...
export default function App() {
  ...
  const handleContinueTelegram = () => {
    ...
    if (!isResultAlreadySent()) {
      sendResultToBotOnce(payload);
    }
